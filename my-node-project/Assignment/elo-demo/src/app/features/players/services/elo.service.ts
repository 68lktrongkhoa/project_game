...
export class EloService { // Bạn có thể cân nhắc đổi tên thành PlayerService nếu nó quản lý nhiều hơn ELO
  ...
  getPlayers(): Observable<Player[]> {
    return this.http.get<Player[]>(`${this.apiUrl}/players`)
      .pipe(
        tap(players => console.log(`Fetched ${players.length} players from backend`)),
  ...
  getPlayerById(playerId: string): Observable<Player> {
    return this.http.get<Player>(`${this.apiUrl}/players/${playerId}`)
      .pipe(
        tap(player => console.log(`Fetched player by ID ${playerId}:`, player)),
        catchError(this.handleError)
      );
  }
  ...
  findOpponent(currentPlayerId: string, eloRange: number): Observable<Player | null> {
    ...
    return this.http.get<Player | null>(`${this.apiUrl}/players/find-opponent`, { params })
      .pipe(
        tap(opponent => console.log('Opponent found via backend:', opponent)),
        catchError(this.handleError)
      );
  }
  ...
  createMatch(
    ...
    return this.http.post<Match>(`${this.apiUrl}/matches`, matchData)
      .pipe(
        tap(newMatch => console.log('Match created via backend:', newMatch)),
        catchError(this.handleError)
      );
  }
  ...
  getMatches(): Observable<Match[]> {
    return this.http.get<Match[]>(`${this.apiUrl}/matches`)
      .pipe(
        tap(matches => console.log(`Fetched ${matches.length} matches from backend`)),
        catchError(this.handleError)
      );
  }
  ...
  getPlayerMatchHistory(playerId: string): Observable<Match[]> {
    return this.http.get<Match[]>(`${this.apiUrl}/matches/player/${playerId}`)
      .pipe(
        tap(matches => console.log(`Fetched match history for player ${playerId}:`, matches.length)),
        catchError(this.handleError)
      );
  }
