...
const DATA_DIRECTORY = path_1.default.join(__dirname, '../data');
function seedData(config) {
    const { entityName, fileName, transformAndValidate, addToService } = config;
    const filePath = path_1.default.join(DATA_DIRECTORY, fileName);
    try {
        ...
        rawItems.forEach((rawItem, index) => {
            try {
                const modelData = transformAndValidate(rawItem, index, fileName);
                if (modelData) {
...
function transformAndValidateBook(rawData, index, fileName) {
    ...
    if (genre !== undefined) {
        if (typeof genre === 'string' && Object.values(book_1.GenreValues).includes(genre)) {
            validatedGenre = genre;
        }
        else {
    ...
    let validatedTotalCopies = undefined;
...
function transformAndValidateUser(rawData, index, fileName) {
...
function loadInitialData(bookService, userService) {
    seedData({
        entityName: 'Sách',
